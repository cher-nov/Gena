  ...
  GVTEST_UNUSED(user_data_or_fixture);
{
  ...
  v_val = gvec_intv_assign( v_val, TEST_SET_LEN, CUSTOM_VALUE_1 );
  munit_assert_true( gvec_error(v_val) == GVEC_ERR_NO );
  v_ref = gvec_set( v_ref, v_val );
  munit_assert_true( gvec_error(v_ref) == GVEC_ERR_NO );

  munit_assert_size( gvec_count(v_val), ==, gvec_count(v_ref) );
  munit_assert_size( gvec_size(v_val), ==, gvec_size(v_ref) );
  munit_assert_size( GVEC_HDR(v_val)->unitsz, ==, GVEC_HDR(v_ref)->unitsz );
  ...
  v_val = gvec_shrink(v_val);
  munit_assert_true( gvec_error(v_val) == GVEC_ERR_IDLE );
  v_ref = gvec_shrink(v_ref);
  munit_assert_true( gvec_error(v_ref) == GVEC_ERR_NO );

  v_ref = gvec_reserve( v_ref, TEST_SET_LEN-gvec_size(v_ref) );
  munit_assert_true( gvec_error(v_ref) == GVEC_ERR_NO );

  for(i = 0; i < TEST_SET_LEN; ++i) {
    v_ref = gvec_intr_push( v_ref, &CUSTOM_VALUE_2 );
    munit_assert_true( gvec_error(v_ref) == GVEC_ERR_NO );
  }
  ...
  v_val = gvec_shrink(v_val);
  munit_assert_true( gvec_error(v_val) == GVEC_ERR_NO );
  v_val = gvec_intv_resize( v_val, TEST_SET_LEN, CUSTOM_VALUE_2 );
  munit_assert_true( gvec_error(v_val) == GVEC_ERR_NO );
  munit_assert_size( gvec_count(v_val), ==, TEST_SET_LEN );
  munit_assert_memory_equal( sizeof(TEST_SET), v_val, v_ref );

  v_val = gvec_set( v_val, NULL );
  munit_assert_null(v_val);
  v_val = gvec_copy(v_ref);
  munit_assert_not_null(v_val);

  for(i = 0; i < TEST_SET_LEN; ++i) {
    rpint = gvec_at(v_ref, i);
    munit_assert_false( gvec_error(v_ref) == GVEC_ERR_IDLE );
  ...
  v_val = gvec_clear(v_val);
  munit_assert_true( gvec_error(v_val) == GVEC_ERR_NO );
  munit_assert_size( gvec_count(v_val), ==, 0 );
  v_val = gvec_set( v_val, NULL );
  munit_assert_null(v_val);

  v_ref = gvec_set( v_ref, NULL );
  munit_assert_null(v_ref);
